...
double LogisticRegression::UpdateThetaVec(Sample &theSample, vector<double> & ClassProbVec, double dLearningRate)
{
	double dCost = 0.0;
		...
		)
{
	...
	for (int iLoop = 0; iLoop < iMaxLoop; ++iLoop)
	{
		...
		for (int i = 0; i < iSampleNum; i++)
		{
			...
			dCost += UpdateThetaVec(SampleVec[i], ClassProbVec, dLearningRate);
		}
...
void LogisticRegression::Test()
{
	...
	TrainSGDOnSampleFile("./data/train.txt", 0.01, 100, 0.001);
	SaveLRModelTxt("./model/model.txt");
	LoadLRModelTxt("./model/model.txt");
